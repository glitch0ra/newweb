class Carousel {
    ...
    updateTransform() {
        ...
        const firstItem = this.items[0];
            ...
            if (firstItem && firstItem.classList.contains('history-image')) {
                ...
                // Get actual width from element, accounting for padding
                const containerWidth = this.container.offsetWidth;
                const padding = parseFloat(getComputedStyle(this.container).paddingLeft) + 
                              parseFloat(getComputedStyle(this.container).paddingRight);
                const availableWidth = containerWidth - padding;
                itemWidth = availableWidth / this.options.itemsPerView;
            } else {
                const containerWidth = this.container.offsetWidth;
    ...
    update() {
        ...
        if (firstItem && firstItem.classList.contains('history-image')) {
            ...
            // Fixed width for collection images - account for padding
            const containerWidth = this.container.offsetWidth;
            const padding = parseFloat(getComputedStyle(this.container).paddingLeft) + 
                          parseFloat(getComputedStyle(this.container).paddingRight);
            const availableWidth = containerWidth - padding;
