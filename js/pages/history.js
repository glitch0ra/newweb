class HistoryPage {
  ...
  async load() {
    try {
      this.data = await dataLoader.loadHistory();
      this.render();
    } catch (error) {
      this.renderError();
    }
      ...
      <div class="history-entry glass">
        <div class="history-grid">
          ...
          <div class="history-content">
            ...
            <p class="history-date">${new Date(entry.date).toLocaleDateString('ru-RU')}</p>
  ...
  setupCarousels() {
    document.querySelectorAll('.history-carousel').forEach(carouselEl => {
      new Carousel(carouselEl, { itemsPerView: 1, loop: true, draggable: true });
    });
  }
